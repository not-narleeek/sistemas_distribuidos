...
hdfs-init:
$(COMPOSE) exec -T namenode bash -lc "hdfs dfs -mkdir -p /data/input/yahoo /data/input/llm /data/output/yahoo /data/output/llm /data/output/compare"
...
$(COMPOSE) exec -T namenode bash -lc "hdfs dfs -mkdir -p /data/input/yahoo /data/input/llm"
$(COMPOSE) exec -T namenode bash -lc "hdfs dfs -put -f /tmp/yahoo_respuestas.csv /data/input/yahoo/"
$(COMPOSE) exec -T namenode bash -lc "hdfs dfs -put -f /tmp/llm_respuestas.csv /data/input/llm/"

$(OUTPUT_DIR)/yahoo_respuestas.csv $(OUTPUT_DIR)/llm_respuestas.csv:
ifndef DUMP_PATH
$(error DUMP_PATH variable is required. Example: make load-data DUMP_PATH=data/respuestas.csv)
endif
...
$(COMPOSE) exec -T namenode bash -lc "rm -rf /tmp/batch-artifacts && mkdir -p /tmp/batch-artifacts && hdfs dfs -get -f /data/output /tmp/batch-artifacts/"
...
$(COMPOSE) exec -T namenode bash -lc "rm -rf /tmp/batch-artifacts"
